...
void drawRotFrame() { // drawFrame()과 흡사하다. 회전하는 방향(angleX, ...)에 대응하는 회전축을 흰색으로 그린다. 길이는 10이고 중심은 소의 모델링 공간 (0,0,0)에 있다.
	if (isRotate != 0) { // 즉, 소가 돌지 않고 멈춰있다면 회전축을 그릴 필요가 없다.
		...
		glColor3d(1, 1, 1);
		if (norm != 0.0) {
			glVertex3d(-5 * angleX / norm, -5 * angleY / norm, -5 * angleZ / norm); // 벡터를 정규화한 뒤 알맞은 길이를 곱해준다.
...
void onMouseDrag( int x, int y ) {
	...
	glPushMatrix();
		...
		if (modelOrView == 0) { //만약 modeling space에서 이동을 원한다면,
			...
			//cow2wld 행렬을 로드했으므로, 위 arguments 의 값은 소 좌표계 기준이다. 이 translation은 world에 반영된다.
		}
...
void onKeyPress( unsigned char key, int x, int y ) {
    ...
    if ( ( key == ' ' ) || ( key == 'c' ) ) {
        ...
        cameraIndex += 1;
		//edit
		if ( isRotate == 1 && modelOrView == 1 ) selectRotationAxis(); // viewing space에서 회전하는 중, 카메라를 변경할 때 회전축도 같이 바꿔준다.
		//end
	}
	else if ((key >= '0') && (key <= '9')) {
		cameraIndex = key - '0';
